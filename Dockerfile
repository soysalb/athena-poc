...
RUN pip install pandas_profiling trino scikit-optimize impyute --no-cache-dir && \ 
	...
	pip install cufflinks --no-cache-dir && \
	pip install dash-bootstrap-components --no-cache-dir && \
	pip install dataframe-image --no-cache-dir && \
...
#	pip install fbprophet --no-cache-dir && \
	...
	pip install imblearn --no-cache-dir && \
	pip install ing-theme-matplotlib --no-cache-dir && \
	pip install ITU-Turkish-NLP-Pipeline-Caller --no-cache-dir && \
	pip install JPype1 --no-cache-dir && \
	pip install Keras --no-cache-dir && \
	pip install Keras-Applications --no-cache-dir && \
